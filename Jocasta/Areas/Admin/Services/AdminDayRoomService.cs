...
namespace Jocasta.Areas.Admin.Services
{
    public class AdminDayRoomService : BaseService
    {
        ...
        public object GetListDayRoomByDay(string keyword, string status, long startDate, long endDate, int floor, IDbTransaction transaction = null)
        {
            ...
            {
                //DateTime StartDate = Convert.ToDateTime(startDate);
                DateTime StartDate = HelperProvider.GetDateTime(startDate);
                long start = HelperProvider.GetSeconds(new DateTime(StartDate.Year, StartDate.Month, StartDate.Day, 0, 0, 0));
                query += " and dr.DayTime >= " + start;
            }
            ...
            {
                //DateTime EndDate = Convert.ToDateTime(endDate);
                DateTime EndDate = HelperProvider.GetDateTime(endDate);
                long end = HelperProvider.GetSeconds(new DateTime(EndDate.Year, EndDate.Month, EndDate.Day, 23, 59, 59));
                query += " and dr.DayTime <= " + end;
            }
            if (!string.IsNullOrEmpty(status))
            ...
            return this._connection.Query<object>(querySelect + query, new {keyword, status, startDate, endDate, floor}, transaction).ToList();
        }
