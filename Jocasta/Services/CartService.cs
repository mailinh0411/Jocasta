...
namespace Jocasta.Services
{
    public class CartService : BaseService
    {
        ...
        {
            string query = "INSERT INTO [dbo].[cart_detail] ([CartDetailId],[CartId],[RoomCategoryId],[Quantity],[ExtraBed],[Price])" +
                " VALUES (@CartDetailId,@CartId,@RoomCategoryId,@Quantity,@ExtraBed,@Price)";
        ...
        {
            string query = "update [cart_detail] set [Quantity] = @Quantity, [ExtraBed] = @ExtraBed, [Price] = @Price where [CartDetailId] = @CartDetailId";
            int status = this._connection.Execute(query, cartDetail, transaction);
            if (status <= 0) throw new Exception(JsonResult.Message.ERROR_SYSTEM);
        }
